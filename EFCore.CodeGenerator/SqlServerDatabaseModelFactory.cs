...
{
    public class MySqlServerDatabaseModelFactory : DatabaseModelFactory
    {
        ...
        /// </summary>
        public MySqlServerDatabaseModelFactory([NotNull] IDiagnosticsLogger<DbLoggerCategory.Scaffolding> logger)
        {
        ...
        private static Func<string, string> GenerateSchemaFilter(IReadOnlyList<string> schemas)
        {
            return schemas.Count > 0
                ? (s =>
                {
                    var schemaFilterBuilder = new System.Text.StringBuilder();
...
ORDER BY [table_schema], [table_name], [f].[name], [fc].[constraint_column_id]";
            ...
            this.GetForeignKeysFromConfig(tables);
        }

        /// <summary>
        /// TODO: NOT support multiple keys.
        /// </summary>
        ...
        private void GetForeignKeysFromConfig(IReadOnlyList<DatabaseTable> tables)
        {
            ...
            foreach (var @class in EFCore.CodeGenerator.Entity.Dapper.Utilities.DbSetting.Classes)
            {
                foreach (var property in @class.Properties.Where(o => !string.IsNullOrEmpty(o.FK)))
                {
                    ...
                    var table = tables.Single(t => t.Name.Equals(@class.TableName, StringComparison.InvariantCultureIgnoreCase));
